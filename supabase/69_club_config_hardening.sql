-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- 69 — DURCISSEMENT upsert_club_config (audit tour 1 du hotfix créneaux, app EN LIGNE)
-- Corrige 4 trous confirmés par l'audit adversarial :
--   (a) `"d":"60"` (chaîne JSON) était ACCEPTÉ par le serveur mais résolu 90 par le client
--       → grille divergente client↔serveur. On exige un NOMBRE jsonb strict (60 | 90).
--   (b) le miroir `slots` n'était dérivé QUE des créneaux 90 min : « Tout en 1h » sur tous les
--       terrains produisait un miroir VIDE → « Repasser aux horaires simples » retombait sur la
--       grille générique. Le miroir devient une ÉCHELLE 90 recouvrant [ouverture, fermeture]
--       de la grille réelle (il ne sert qu'aux bornes tarifaires et au retour au mode simple).
--   (c) AUCUNE garde serveur contre l'« orphelinage » d'une réservation payée : une grille 'set'
--       (ou un 'clear') qui ne fait plus retomber une résa À VENIR ('booked') exactement sur un
--       créneau (heure + durée) de son terrain est désormais REFUSÉE — la garde client (miroir
--       local, possiblement périmé) n'est plus le seul rempart.
--   (d) au 'set', si l'échelle dérivée est vide (plage < 1h30), on PRÉSERVE l'ancien miroir au
--       lieu d'écrire un tableau vide.
-- Idempotent : redéfinition COMPLÈTE de la fonction 68 (même signature → create or replace).
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
...
begin
  ...
  select cc0.slots into v_prev_slots from public.club_config cc0 where cc0.club_id = p_club_id;
