-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- 71 — VERROU COMMUN garde anti-orphelin ↔ insertion de réservation (audit tour 2)
-- La garde (69/70) vérifiait les résas PUIS écrivait la grille sans verrou commun avec
-- reservations_insert_guard : une réservation concurrente pouvait se glisser entre les deux et
-- naître orpheline malgré la garde. Les deux chemins prennent désormais le MÊME verrou
-- consultatif de club (pris en PREMIER des deux côtés → ordre constant, pas d'interblocage).
-- Idempotent : redéfinitions COMPLÈTES (70 pour upsert_club_config, base live pour le trigger).
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
