...
--      ({ "Terrain 1": ["18:00"] } = le Terrain 1 est fermé tous les jours à 18h00, ex. réservé
--      aux cours). Les autres terrains restent réservables à cette heure.
--
...
begin
  ...
  if p_times is not null and exists (select 1 from unnest(p_times) t where t !~ '^\d{2}:\d{2}$') then
    return 'invalid';
  end if;
  ...
  if exists (
    ...
    where r.club_id = p_club_id and r.status = 'booked' and r.starts_at > v_now
      ...
      and (p_court is null or r.court = p_court)
  ...
  insert into public.blocked_ranges (club_id, court, date_from, date_to, times, reason)
    values (p_club_id, nullif(trim(coalesce(p_court, '')), ''), p_date_from, p_date_to,
